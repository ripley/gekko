...
const telegrambotplus = config.telegrambotplus;
const emitTrades = telegrambotplus.emitTrades;
...
if(emitTrades) {
  Actor.prototype.processTradeInitiated = function (tradeInitiated) {
    let message = '<b>Trade initiated.</b>' +
...
Actor.prototype.verifyQuestion = function(msg, text) {
  //determine whether in user or admin mode first
  console.log(text);
  ...
  if (user === undefined) {
     ...
     //whether waiting for admin pw or already in admin mode
...
Actor.prototype.emitAdminPortfolio = function(chatId) {
  let message = [
    ...
    'Change since last trade: <b>' + (!!this.pperformance ? this.pperformance : 'Don\'t know yet :(') + '</b>\n'
  ].join('');
